...
data class HttpClientConfig(
    ...
    // 处理特殊code
    var codes: ArrayList<Int> = arrayListOf(),
    //restful 规则格式
    var tData: Map<String, String> = mapOf(),
    //特殊情况接口回调
    var callback: SpecialCallback? = null,
    // 请求头
...
class HttpClientConfigBuilder {
    ...
    fun setSpecialCallback(callback: SpecialCallback?): HttpClientConfigBuilder {
        config.callback = callback
        return this
    }

    fun setCodes(codes: ArrayList<Int>): HttpClientConfigBuilder {
        config.codes = codes
        return this
    }
    fun setTData(tData: Map<String, String>): HttpClientConfigBuilder {
        config.tData = tData
        return this
    }
